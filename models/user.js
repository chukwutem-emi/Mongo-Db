...
class User {
  ...
  addToCart(product) {
    if (!product || !product._id) {
      throw new Error("Invalid product passed to addToCart");
    }
    this.cart = this.cart || {items: []}
    this.cart.items = Array.isArray(this.cart.items) ? this.cart.items : [];
    this.cart.items = this.cart.items.filter(cp => cp.productId);
